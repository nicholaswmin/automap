...
test('#flatten()', async t => {
  ...
  await t.test('Nested List - value', async t => {
    ...
    await t.test('value has key matching the path of the list', () => {
      assert.strictEqual(list.key, 'building:foo:flats:101:visitors')
    })
    ...
    await t.test('each key in value maps to an id of an array item', () => {
      assert.deepStrictEqual(Object.keys(list.value), ['p1', 'p2'])
    })

    await t.test('visitor 1 has an index at 0', () => {
      const parsed = JSON.parse(list.value['p1'])
      assert.strictEqual(parsed.i, 0)
    })

    await t.test('visitor 2 has an index at 1', () => {
      const parsed = JSON.parse(list.value['p1'])
      assert.strictEqual(parsed.i, 0)
    })

    await t.test('each value has a json of the array item', () => {
      assert.strictEqual(typeof list.value['p1'], 'string')
    })

    await t.test('visitor 1 json matches the visitor 1 array item', () => {
      const parsed = JSON.parse(list.value['p1'])
