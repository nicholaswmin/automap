...
test('#repository.save()', async t => {
  ...
  await t.test('fetching existing object with 1 LazyList item', async t => {
    ...
    await t.test('does not preload the list', t => {
      t.assert.strictEqual(building.visitors.length, 0)
    })
    ...
    await t.test('loading its list once', async t => {
      ...
      await t.test('with the correct type', t => {
        t.assert.ok(building.visitors.at(0) instanceof Person, 'not a Building')
      })
