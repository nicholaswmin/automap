...
test('AppendList, nested, 1-level, add items: x 10000 times', async t => {
  ...
  await t.test('create 200 Lists, each nesting 1 AppendList', async t => {
    for (let i = 0; i < 200; i++) {
    ...
    await t.test('for each AppendList', async t => {
      await t.test('x 50 times', async t => {
        await t.test('add 1 AppendList item & save()', async t => {
          ...
          await t.test('fetch()', async t => {
            await t.test('is called 10000 times', t => {
              const count = fetch.stats_ms.count

              t.assert.strictEqual(count, 10000, `ran: ${count} times`)
            })
