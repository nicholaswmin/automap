...
test('perf: add 10k AppendList items ~ 3 kb, nested in 100 Lists', async t => {
  ...
  await t.before(() => {
    console.info(col('yellow',
      '* The following test could take > 10 minutes to run. Please wait ... *',
    ))
  })
    ...
    await t.test('run 100 times, add a List item in each', async t => {
      await t.test('run 100 times, add an AppendList item in each', async t => {
        ...
        await t.test('3 randomly-picked, saved AppendLists', async t => {
          // - picking notes of 3 random users
          const lists = await Promise.all([1,50,95].map(uid => {
            return redis.lrange(`chatroom:foo:users:${uid}:notes`, 0, -1)
          }))
