...
test('performance: List', async t => {
  ...
  await t.test('starts with 100 items, ~ 0.1 kb each', async t => {
    ...
    await t.test('edits an item per cycle from ~ 0.1 kb to ~ 1 kb', async t => {
      await t.test('runs 100 fetch/save cycles', async t => {
        ...
        await t.beforeEach(async () => {
          ...
          for (let i = 0; i < 100; i++) {
            ...
            if (room?.users.at(i))
              room.users.at(i).name = utils.payloadKB(1)
        ...
        await t.test('durations', async t => {
          await t.test('#fetch', async t => {
            await t.test('ran specified cycles', () => {
            ...
            await t.test('max is under 10 ms', () => {
          ...
          await t.test('#save', async t => {
            ...
            await t.test('max is under 15 ms', () => {
              const ms = histograms.save.max / 1e+6

              assert.ok(ms < 15, `value is: ${ms} ms`)
            })

            await t.test('deviation is under 4 ms', () => {
